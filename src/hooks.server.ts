...
const authGuard: Handle = async ({ event, resolve }) => {
	...
	if (!event.locals.session && event.url.pathname.startsWith('/dashboard/auth/private')) {
		redirect(303, '/dashboard/auth');
	}

	if (event.locals.session && event.url.pathname === '/dashboard/auth') {
